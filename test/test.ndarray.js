...
tape( 'the function calculates the dot product of arrays `x` and `y` (accessors)', function test( t ) {
	...
	x = toAccessorArray( [ 4.0, 2.0, -3.0, 5.0, -1.0, 2.0, -5.0, 6.0 ] );
	y = toAccessorArray( [ 2.0, 6.0, -1.0, -4.0, 8.0, 8.0, 2.0, -3.0 ] );
...
tape( 'the function supports an `x` stride (accessors)', function test( t ) {
	...
	dot = gdot( 3, toAccessorArray( x ), 2, 0, toAccessorArray( y ), 1, 0 );
	t.strictEqual( dot, -12.0, 'returns expected value' );
	t.end();
});
...
tape( 'the function supports an `x` offset (accessors)', function test( t ) {
	...
	dot = gdot( 3, toAccessorArray( x ), 2, 1, toAccessorArray( y ), 1, 0 );
	t.strictEqual( dot, 88.0, 'returns expected value' );
	t.end();
});
...
tape( 'the function supports a `y` stride (accessors)', function test( t ) {
	...
	dot = gdot( 3, toAccessorArray( x ), 1, 0, toAccessorArray( y ), 2, 0 );
	t.strictEqual( dot, 45.0, 'returns expected value' );
	t.end();
});
...
tape( 'the function supports a `y` offset (accessors)', function test( t ) {
	...
	dot = gdot( 3, toAccessorArray( x ), 2, 0, toAccessorArray( y ), 1, 3 );
	t.strictEqual( dot, 94.0, 'returns expected value' );
	t.end();
});
...
tape( 'the function supports negative strides (accessors)', function test( t ) {
	...
	dot = gdot( 3, toAccessorArray( x ), -2, x.length-1, toAccessorArray( y ), -1, 2 ); // eslint-disable-line max-len
	t.strictEqual( dot, 67.0, 'returns expected value' );
	t.end();
});
...
tape( 'the function supports complex access patterns (accessors)', function test( t ) {
	...
	dot = gdot( 3, toAccessorArray( x ), 2, 0, toAccessorArray( y ), -1, y.length-2 ); // eslint-disable-line max-len
	t.strictEqual( dot, 68.0, 'returns expected value' );
	t.end();
});
...
tape( 'if both strides are equal to `1`, the function efficiently calculates the dot product (accessors)', function test( t ) {
	...
	dot = gdot( x.length, toAccessorArray( x ), 1, 0, toAccessorArray( y ), 1, 0 ); // eslint-disable-line max-len
